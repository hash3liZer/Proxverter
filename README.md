...
Likewise, if you want to suppress all the errors from `proxy.py`, you can supply `suppress_errors=True` to Proxverter instance: 
```python
import proxverter
prox = proxverter.Proxverter(ip="127.0.0.1", port=8081, verbose=True, suppress_errors=True)
prox.engage()
```
